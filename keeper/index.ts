 ...
 *
 * Continuously scans for cycles that start AND end in the SAME token -- 2 hops
 * up to MAX_HOPS (default 12, hard-capped at 16; keeper/.env currently pins it
 * to 4). Executes them atomically through the already-deployed AeonArbKeeper
 * contract instead of raw sequential swaps.
 *
 * WHERE THE POOLS COME FROM depends on KEEPER_ROLE, and the two paths are not
 * the same list:
 *   - default role: ARB_POOLS is derived from POOLS in src/config/contracts.ts
 *     (the same list the website reads) plus on-chain discovery.
 *   - MIRAJANE_MODE (KEEPER_ROLE=mirajane, the live setting): loadMirajanePools()
 *     SPLICES THAT SET AWAY and replaces it with keeper/mirajane-pools.json --
 *     a curated AEON + Uniswap V3/V4 manifest generated by gen-mirajane-pools.mjs,
 *     with no discovery at all. So in the live configuration there IS a separate
 *     pool list and it CAN drift from the website; regenerate it rather than
 *     assuming an edit to contracts.ts reaches the bot.
 *
 * Cycles may only start and end in SETTLEMENT_TOKENS (AEON, USDG, WETH, and
 * CASHCAT since 2026-08-01). Any other token can be routed THROUGH but can
 * never anchor a cycle, so holding it is not trading capital. The search runs
 * once per settlement token that has a balance, which is why adding one is a
 * real cost rather than a free win.
 *
...
async function tick(changedPoolKeys?: Set<string>, observedBlock?: bigint) {
  ...
  if (!poolDiscoveryInFlight && Date.now() - lastPoolRefresh >= POOL_REFRESH_INTERVAL_MS) {
    ...
    void (async () => {
      ...
      // Force a full pool-state re-read on the next tick, even when discovery
      // added nothing. Incremental scans only re-read pools whose events were
      // observed since the previous tick; a manually-seeded V4 pool (the FRONG
      // pools at the top of this file) can drain to ZERO liquidity without the
      // change-detector ever flagging it, so its stale nonzero reserves stay
      // cached indefinitely. The zero-liquidity guard in fetchPoolStates only
      // runs on a real re-read, so until one happens the pool keeps producing a
      // phantom arb edge that passes local sizing and then reverts on-chain as
      // "output moved below the protected minimum". Observed 2026-08-01: ~11h of
      // FRONG routes quoting 1-3% and failing that way, cleared only by a manual
      // restart (which full-reads everything on boot). Invalidating here bounds
      // any such staleness to one POOL_REFRESH_INTERVAL_MS so it self-heals.
