...
// Two workers keep the single Robinhood RPC busy without creating the large
// bursts that previously triggered provider throttling. Clamp configuration
// so an accidental value cannot turn preflight into an RPC flood.
const EXACT_PREFLIGHT_CONCURRENCY = Math.max(1, Math.min(2, parseInt(process.env.EXACT_PREFLIGHT_CONCURRENCY ?? '2')))
...
const POOL_DENYLIST = new Set(
  (process.env.POOL_DENYLIST ?? '')
    .split(',')
...
const POOL_ACTIVITY_CHECK_INTERVAL_MS = parseInt(process.env.POOL_ACTIVITY_CHECK_INTERVAL_MS ?? String(48 * 60 * 60 * 1000))
const POOL_INACTIVE_AFTER_MS = parseInt(process.env.POOL_INACTIVE_AFTER_MS ?? String(50 * 60 * 60 * 1000))
const POOL_ACTIVITY_LOG_CHUNK_BLOCKS = BigInt(process.env.POOL_ACTIVITY_LOG_CHUNK_BLOCKS ?? '20000')
...
// Execution profitability must not depend on an approximate spot conversion.
// Re-quote the buffered native gas cost through the live venue quoters/formula
// using the same fresh on-chain state as the candidate. Local spot conversion
// remains useful for discovery/ranking only; this value is the safety gate.
async function weiToTokenExact(tokenSym: string, amountWei: bigint, graph: Map<string, HopCandidate[]>): Promise<bigint | null> {
  if (tokenSym === 'WETH') return amountWei
  const path = findConversionPath(graph, 'WETH', tokenSym)
  if (!path) return null
  const quoted = await quoteMixedRouteExact(path, amountWei)
  return quoted > 0n ? quoted : null
}
...
type OutcomeKey = 'detected' | 'executed' | 'belowGas' | 'insufficientBalance' | 'simulationFailed' | 'preflightRejected' | 'staleQuote' | 'reverted'
const outcomeCounters: Record<OutcomeKey, number> = {
  detected: 0, executed: 0, belowGas: 0, insufficientBalance: 0,
  simulationFailed: 0, preflightRejected: 0, staleQuote: 0, reverted: 0,
}
...
const scanTelemetry = {
  ...
  exactChecked: 0,
  exactValid: 0,
  lastBlock: null as string | null,
}

type OpportunityValidation = 'discovery' | 'exact-valid' | 'preflight-rejected' | 'cooldown'
interface ReportedOpportunity {
  opp: ArbOpp | SettlementOpp
  validation: OpportunityValidation
  rejectionReason?: string
}
let exactPreflightCursor = 0
...
let poolActivityReport: PoolActivityReport = {
  checkedAt: null,
  nextCheckAt: null,
  inactiveAfterHours: POOL_INACTIVE_AFTER_MS / 3_600_000,
  checkIntervalHours: POOL_ACTIVITY_CHECK_INTERVAL_MS / 3_600_000,
  auditRunning: false,
  pools: [],
  flagged: [],
  unknown: [],
}
let poolActivityAuditRunning = false

async function blockAtOrAfterTimestamp(targetSeconds: bigint, latestBlock: bigint): Promise<bigint> {
  let low = 0n
  let high = latestBlock
  while (low < high) {
    const mid = (low + high) / 2n
    const block = await pub.getBlock({ blockNumber: mid })
    if (block.timestamp < targetSeconds) low = mid + 1n
    else high = mid
  }
  return low
}

async function latestPoolEventBlock(address: `0x${string}`, fromBlock: bigint, toBlock: bigint): Promise<bigint | null> {
  // Robinhood's official RPC can usually answer this sparse 50-hour range in
  // one call. If a busy pool exceeds its response limit, fall back to bounded
  // chunks searched newest-first.
  try {
    const logs = await pub.getLogs({ address, fromBlock, toBlock })
    return logs.length > 0 ? logs[logs.length - 1].blockNumber : null
  } catch { /* range/result limit: use bounded fallback below */ }

  let end = toBlock
  while (end >= fromBlock) {
    const start = end - POOL_ACTIVITY_LOG_CHUNK_BLOCKS + 1n < fromBlock
      ? fromBlock
      : end - POOL_ACTIVITY_LOG_CHUNK_BLOCKS + 1n
    const logs = await pub.getLogs({ address, fromBlock: start, toBlock: end })
...
async function auditPoolActivity() {
  if (poolActivityAuditRunning) return
  poolActivityAuditRunning = true
  poolActivityReport = { ...poolActivityReport, auditRunning: true }
  const checkedAtMs = Date.now()
  try {
    const latestBlock = await pub.getBlockNumber()
    const cutoffSeconds = BigInt(Math.floor((checkedAtMs - POOL_INACTIVE_AFTER_MS) / 1000))
    const cutoffBlock = await blockAtOrAfterTimestamp(cutoffSeconds, latestBlock)
    const blockTimes = new Map<string, string>()
    const entries: PoolActivityEntry[] = []

    // This is deliberately sequential. It runs only every 48 hours and must
    // never create an RPC burst that competes with the latency-sensitive scan.
    for (const pool of ARB_POOLS) {
      try {
        const eventBlock = await latestPoolEventBlock(getAddress(pool.address), cutoffBlock, latestBlock)
        let lastActivityAt: string | null = null
        let inactiveHours = POOL_INACTIVE_AFTER_MS / 3_600_000
        if (eventBlock != null) {
          const key = eventBlock.toString()
          let time = blockTimes.get(key)
          if (!time) {
            const block = await pub.getBlock({ blockNumber: eventBlock })
            time = new Date(Number(block.timestamp) * 1000).toISOString()
            blockTimes.set(key, time)
          }
          lastActivityAt = time
          inactiveHours = Math.max(0, (checkedAtMs - new Date(time).getTime()) / 3_600_000)
        }
        const flagged = eventBlock == null
        entries.push({
          name: pool.name, address: pool.address, kind: pool.kind,
          token0: pool.token0, token1: pool.token1,
          lastActivityAt, inactiveHours, flagged,
          state: flagged ? 'inactive' : 'active',
        })
      } catch (err: any) {
        entries.push({
          name: pool.name, address: pool.address, kind: pool.kind,
          token0: pool.token0, token1: pool.token1,
          lastActivityAt: null, inactiveHours: 0, flagged: false, state: 'unknown',
          error: String(err?.shortMessage ?? err?.message ?? err).slice(0, 180),
        })
      }
    }
    ...
    poolActivityReport = {
      checkedAt,
      nextCheckAt: new Date(checkedAtMs + POOL_ACTIVITY_CHECK_INTERVAL_MS).toISOString(),
      inactiveAfterHours: POOL_INACTIVE_AFTER_MS / 3_600_000,
      checkIntervalHours: POOL_ACTIVITY_CHECK_INTERVAL_MS / 3_600_000,
      auditRunning: false,
      pools: entries,
      flagged: entries.filter(entry => entry.flagged),
      unknown: entries.filter(entry => entry.state === 'unknown'),
    }
    console.log(`[pool activity] checked ${entries.length} pools: ${poolActivityReport.flagged.length} inactive, ${poolActivityReport.unknown.length} unknown`)
  } catch (err: any) {
    const message = `Pool activity audit failed: ${err?.shortMessage ?? err?.message ?? String(err)}`
    console.error(`[pool activity error] ${message}`)
    recentErrors.unshift({ time: new Date().toISOString(), message })
    recentErrors = recentErrors.slice(0, 5)
    poolActivityReport = { ...poolActivityReport, auditRunning: false }
  } finally {
    poolActivityAuditRunning = false
  }
}

function maybeAuditPoolActivity() {
  const lastCheck = poolActivityReport.checkedAt ? new Date(poolActivityReport.checkedAt).getTime() : 0
  const auditedPools = new Set(poolActivityReport.pools.map(pool => `${pool.kind}:${pool.address.toLowerCase()}`))
  const currentPools = new Set(ARB_POOLS.map(pool => `${pool.kind}:${pool.address.toLowerCase()}`))
  const universeChanged = auditedPools.size !== currentPools.size || Array.from(currentPools).some(pool => !auditedPools.has(pool))
  if (!poolActivityAuditRunning && (universeChanged || Date.now() - lastCheck >= POOL_ACTIVITY_CHECK_INTERVAL_MS)) {
    void auditPoolActivity()
  }
}
...
function countFailureOutcome(failure: DecodedFailure, stage: FailureStage) {
  ...
  if (stage === 'quote' || stage === 'approval' || stage === 'gas_estimate' || stage === 'simulation') outcomeCounters.preflightRejected++
  ...
  else if (stage === 'confirmation') outcomeCounters.reverted++
}
...
try {
  ...
  // Metrics v1 counted quote/gas-estimate rejections as on-chain failures and
  // accumulated every candidate evaluation forever. Start the corrected,
  // receipt-based counters clean instead of carrying those false numbers on.
  if (prior.metricsVersion === METRICS_VERSION) {
    totalFailed = prior.totalArbsFailed ?? 0
    Object.assign(outcomeCounters, prior.outcomeCounters ?? {})
  }
  // Old pre-executor records stored the optimistic discovery percentage on
  // rejected/failed rows. They were never realized profit and must not remain
  // visible beside receipt-confirmed trades after the exact-profit upgrade.
  recentArbs = recentArbs.filter(arb =>
    arb.status === 'success' || arb.status === 'dry-run' || !(arb.profitPct > 0 && arb.profit === '0')
  )
  if (prior.poolActivity?.checkedAt) {
    poolActivityReport = { ...poolActivityReport, ...prior.poolActivity, auditRunning: false }
  }
...
async function writeStatus(lastOpps: ReportedOpportunity[], tickMs: number, rawBalances: Record<string, bigint>, nativeEth: bigint, gasReserveWei: bigint, gasReserveHealthy: boolean, graph: Map<string, HopCandidate[]>) {
  ...
  const status = {
    ...
    balances,
    lastOpportunities: lastOpps.slice(0, 20).map(({ opp: o, validation, rejectionReason }) => {
    ...
    if (hop.pool.pool.kind === 'directCL') {
      ...
      const result = await pub.readContract({
        ...
        args: [{
          ...
          // These pools use Algebra's default factory deployer. The quoter's
          // `deployer` parameter is therefore address(0), not the core
          // PoolDeployer contract. Passing PoolDeployer deterministically
          // derives a different/nonexistent pool and makes every quote revert.
      ...
      const result = await pub.readContract({
        ...
        args: [getAddress(hop.pool.pool.address), amount, hop.tokenInSym === hop.pool.pool.token0],
      }) as readonly [bigint, bigint, bigint]
      const [amountInLeft, amountOut] = result
      if (amountInLeft > 0n) return 0n
      amount = amountOut
    } else {
...
async function preflightCycleExact(
  ...
  const allowanceKey = `${tokenIn.address.toLowerCase()}:${executor.toLowerCase()}`
  let cachedAllowance = allowanceCache.get(allowanceKey)
  if (!cachedAllowance || cachedAllowance.expiresAt <= Date.now()) {
    ...
    cachedAllowance = { value, expiresAt: Date.now() + 60_000 }
  ...
  const deadline = BigInt(Math.floor(Date.now() / 1000) + DEADLINE_SECONDS)
  // The deployed executor, not a composition of independent per-hop quotes,
  // is the authoritative executable output. This also accounts for transfer
  // taxes and repeated-pool state changes that standalone quoters cannot see.
  let exactOut = 0n
  if (mixed) {
    const candidateInputs = [executableCap, executableCap / 8n, executableCap / 64n]
      .filter((amount, index, values) => amount > 0n && values.indexOf(amount) === index)
    let bestInput = 0n
    let bestProfit = 0n
    let lastEconomicRejection: unknown
    for (const candidateInput of candidateInputs) {
      try {
        const initial = await pub.simulateContract({
          account,
          address: CONTRACTS.UniversalRouter, abi: AEON_UNIVERSAL_ROUTER_ABI, functionName: 'swapExactTokensForTokens',
          args: [buildUniversalHops(opp.hops), candidateInput, candidateInput + 1n, account.address, deadline],
        })
        const candidateOut = typeof initial.result === 'bigint' ? initial.result : 0n
        const candidateProfit = candidateOut > candidateInput ? candidateOut - candidateInput : 0n
        const candidateSurplus = candidateProfit > minimumNetProfitRaw(candidateInput)
          ? candidateProfit - minimumNetProfitRaw(candidateInput)
          : 0n
        const bestSurplus = bestProfit > minimumNetProfitRaw(bestInput)
          ? bestProfit - minimumNetProfitRaw(bestInput)
          : 0n
        if (candidateSurplus > bestSurplus) {
        ...
        lastEconomicRejection = err
      }
    }
    if (bestInput > 0n) opp.amountIn = bestInput
    else if (lastEconomicRejection) throw lastEconomicRejection
  } else {
    if (allowance < opp.amountIn) return false
    const initial = await pub.simulateContract({
      account,
      address: CONTRACTS.ArbKeeper, abi: ARB_KEEPER_ABI, functionName: 'executeArb',
      args: [buildKeeperHops(opp.hops), opp.amountIn, 1n, deadline],
    })
    const returnedProfit = typeof initial.result === 'bigint' ? initial.result : 0n
    exactOut = opp.amountIn + returnedProfit
  }
  ...
  if (mixed) {
    executionGas = await pub.estimateContractGas({
      account: account.address,
      address: CONTRACTS.UniversalRouter, abi: AEON_UNIVERSAL_ROUTER_ABI, functionName: 'swapExactTokensForTokens',
      args: [buildUniversalHops(opp.hops), opp.amountIn, opp.amountIn + 1n, account.address, deadline],
    })
  } else {
    executionGas = await pub.estimateContractGas({
      account: account.address,
  ...
  if (mixed) {
    const simulation = await pub.simulateContract({
      account,
      address: CONTRACTS.UniversalRouter, abi: AEON_UNIVERSAL_ROUTER_ABI, functionName: 'swapExactTokensForTokens',
      args: [buildUniversalHops(opp.hops), opp.amountIn, opp.amountIn + requiredGrossProfit, account.address, deadline],
    })
    if (typeof simulation.result === 'bigint' && simulation.result < opp.amountIn + requiredGrossProfit) return false
  } else {
    const simulation = await pub.simulateContract({
      account,
      address: CONTRACTS.ArbKeeper, abi: ARB_KEEPER_ABI, functionName: 'executeArb',
      args: [buildKeeperHops(opp.hops), opp.amountIn, requiredGrossProfit, deadline],
    })
    if (typeof simulation.result === 'bigint' && simulation.result < requiredGrossProfit) return false
  }

  const netProfit = grossProfit - exactGasInToken
  opp.profitRaw = grossProfit
  opp.profitPct = Number(netProfit * 1_000_000n / opp.amountIn) / 10_000
...
async function executeArbViaUniversalRouter(opp: ArbOpp, graph: Map<string, HopCandidate[]>, availableEthForWrap: bigint): Promise<ExecResult> {
  ...
  try {
    ...
    requiredProfit = exactGasInToken + minNetProfit
    ...
    const confirmedFailure = failureStage === 'confirmation'
    if (confirmedFailure) totalFailed++
    ...
    if (confirmedFailure && !failure.routeScoped) consecutiveFailures++
    ...
    recordArb({
      time: new Date().toISOString(), pair: pairLabel, tokenIn: tokenIn.symbol,
      amountIn: formatUnits(amountIn, tokenIn.decimals), profit: '0', profitPct, status: confirmedFailure ? 'failed' : 'rejected', error: message,
...
async function executeArb(opp: ArbOpp, graph: Map<string, HopCandidate[]>, availableEthForWrap: bigint): Promise<ExecResult> {
  ...
  const hops = buildKeeperHops(hopCandidates)
...
async function tick() {
  ...
  let rankedCandidates: RankedInternalCandidate[] = [
  ...
  scanTelemetry.approximateCandidates = rankedCandidates.length
  scanTelemetry.localSearchMs = Date.now() - localSearchStartedAt

  const reportedCandidates: ReportedOpportunity[] = rankedCandidates.slice(0, 20).map(candidate => ({
    opp: candidate.opp,
    validation: routeCooldownRemaining(candidate.opp.hops) > 0 ? 'cooldown' : 'discovery',
  }))

  // Publish the fast discovery result before serial deployed-executor checks.
  // This keeps the operator view fresh and exposes incoming routes while the
  // safety gate continues. Discovery-only rows are never executable; only the
  // exactValidated set below can reach executeArb().
  await writeStatus(reportedCandidates, Date.now() - t0, balances, nativeEth, gasReserveWei, gasReserveHealthy, graph)

  // Discovery deliberately uses cheap reserve math to search broadly, but a
  // CL tick range or DLMM bin distribution cannot be priced as x*y=k. Before
  // either execution or publication, replace every mixed-route estimate with
  // the output returned by the deployed venue quoter. A rejected quote is a
  // harmless preflight rejection, never a failed arbitrage transaction.
  // Robinhood's public endpoint is shared with the other protocol keepers.
  // Run the complete exact set serially so full coverage never becomes a
  // burst of eth_call/estimateGas requests that trips its minute window.
  // Bound each scan's RPC work while preserving both priority and complete
  // coverage over time. The four best candidates are always checked; four
  // additional candidates rotate through the remainder. This avoids losing
  ...
  if (eligibleForPreflight.length > EXACT_PREFLIGHT_BUDGET) {
    ...
    const remainder = eligibleForPreflight.slice(4)
    const rotating: RankedInternalCandidate[] = []
    for (let i = 0; i < 4; i++) rotating.push(remainder[(exactPreflightCursor + i) % remainder.length])
    exactPreflightCursor = (exactPreflightCursor + 4) % remainder.length
    candidatesToPreflight = [...priority, ...rotating]
  }
  scanTelemetry.exactSelected = candidatesToPreflight.length
  const exactQuoteStartedAt = Date.now()
  const exactValidated = await mapWithConcurrency(candidatesToPreflight, EXACT_PREFLIGHT_CONCURRENCY, async candidate => {
    if (candidate.kind !== 'cycle') {
      return (candidate.opp.expectedNetUsd ?? -Infinity) > 0 ? candidate : null
    }
    // Do not spend the shared RPC quota re-simulating a route that the
    // deployed executor just disproved. The existing bounded cooldown is
    // route-local; other routes remain eligible, and this route is retried
    // automatically after fresh blocks have had time to change its economics.
    ...
    try {
      ...
      if (valid) clearRouteFailure(candidate.opp.hops)
        ...
        message: 'Exact route does not clear deployed-executor, gas, and net-profit gates',
        routeScoped: true,
      })
      ...
      outcomeCounters.preflightRejected++
      const failure = decodeFailure(err)
      registerRouteFailure(candidate.opp.hops, failure)
      const message = `[exact_quote/preflight] ${err?.shortMessage ?? err?.message ?? String(err)}`
      const report = reportedCandidates.find(item => item.opp === candidate.opp)
      if (report) {
        report.validation = 'preflight-rejected'
        report.rejectionReason = (err?.shortMessage ?? err?.message ?? String(err)).slice(0, 180)
      }
      // These two custom errors mean the deployed executor rejected an
      // optimistic discovery quote. That is a normal preflight rejection,
      // not an operational failure. Keep real RPC/nonce/encoding faults in
      // recentErrors, while counting these safely rejected candidates only.
      if (!message.includes('0xa5adf0af') && !message.includes('0xbb2875c3')) {
        recentErrors.unshift({ time: new Date().toISOString(), message: message.slice(0, 300) })
        recentErrors = recentErrors.slice(0, 5)
      }
      return null
    }
  })
  scanTelemetry.exactQuoteMs = Date.now() - exactQuoteStartedAt
  rankedCandidates = exactValidated.filter((candidate): candidate is RankedInternalCandidate => candidate !== null)
  scanTelemetry.exactValid = rankedCandidates.length
  rankedCandidates.sort((a, b) => (b.opp.routeScore ?? -Infinity) - (a.opp.routeScore ?? -Infinity))
  ...
  await writeStatus(reportedCandidates, tickMs, balances, nativeEth, gasReserveWei, gasReserveHealthy, graph)
}
...
async function refreshPoolDiscovery(): Promise<DiscoveryCounts> {
  ...
  for (let i = ARB_POOLS.length - 1; i >= 0; i--) {
    const address = ARB_POOLS[i].address.toLowerCase()
    if (POOL_DENYLIST.has(address) || (POOL_ALLOWLIST.size > 0 && !POOL_ALLOWLIST.has(address))) {
