...
const GAUGE_ABI = parseAbi(['function fees0() view returns (uint256)','function fees1() view returns (uint256)'])
...
for (const pool of poolAddrs) {
  ...
  if (!t0||!t1||!gauge||gauge==='0x0000000000000000000000000000000000000000') continue
  const [tk0,tk1] = await Promise.all([getToken(t0),getToken(t1)])

  const [f0,f1] = await Promise.all([rd(gauge,GAUGE_ABI,'fees0'),rd(gauge,GAUGE_ABI,'fees1')])
  if (!f0&&!f1) continue
...
console.log(`=== Unswept fees sitting in gauge contracts (fees0/fees1) ===`)
for (const r of rows) console.log(`  ${r.name.padEnd(22)} $${r.usd.toFixed(2).padStart(9)}   ${r.d}`)
console.log(`\nTOTAL unswept gauge fees: $${grandTotal.toFixed(2)}`)
