  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-12">
      ...
      >
        ...
        {/* Quote info & Visual Route Diagram */}
        {hasAmount && (route || use1inch) && amountOutWei > 0n && (
          <div className="mt-3 mx-1 p-3 bg-bg-base rounded-xl space-y-2 border border-bg-border/60">
            {/* Route Hop Diagram */}
            <div className="p-2.5 bg-bg-raised/70 rounded-lg border border-bg-border/40">
              <div className="flex items-center justify-between text-2xs font-mono text-text-muted mb-2">
                <span>Routing Breakdown</span>
                <span className="text-aeon-400 font-semibold">{use1inch ? '1inch Best Price' : route?.split ? 'AEON Priority Split' : route?.steps.length === 1 ? 'Direct Hop' : 'Multi-hop Route'}</span>
              </div>
              <div className="flex items-center gap-1.5 overflow-x-auto py-1 no-scrollbar text-xs">
                <span className="px-2 py-1 rounded bg-bg-base border border-bg-border font-mono font-semibold text-text-primary shrink-0">{TOKENS[tokenIn].symbol}</span>
                {!use1inch && route?.steps.map((step, idx) => (
                  <div key={idx} className="flex items-center gap-1.5 shrink-0">
                    <span className="text-text-muted font-mono text-2xs">→</span>
                    <span className={clsx(
                      'px-2 py-0.5 rounded text-2xs font-mono font-medium border',
                      step.poolType === 0 ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' :
                      step.poolType === 1 ? 'bg-violet-500/10 border-violet-500/30 text-violet-400' :
                      step.poolType === 2 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' :
                      'bg-amber-500/10 border-amber-500/30 text-amber-400'
                    )}>
                      {step.poolType === 0 ? `vAMM (${(Number(step.feeBps) / 100).toFixed(1)}%)` :
                       step.poolType === 1 ? 'Algebra CL' :
                       step.poolType === 2 ? 'TraderJoe DLMM' : 'External V2'}
                    </span>
                    <span className="text-text-muted font-mono text-2xs">→</span>
                    <span className="px-2 py-1 rounded bg-bg-base border border-bg-border font-mono font-semibold text-text-primary shrink-0">{step.tokenOut}</span>
                  </div>
                ))}
                {use1inch && (
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className="text-text-muted font-mono text-2xs">→</span>
                    <span className="px-2 py-0.5 rounded text-2xs font-mono font-medium bg-amber-500/10 border border-amber-500/30 text-amber-400">1inch V6 Aggregator</span>
                    <span className="text-text-muted font-mono text-2xs">→</span>
                    <span className="px-2 py-1 rounded bg-bg-base border border-bg-border font-mono font-semibold text-text-primary shrink-0">{TOKENS[tokenOut].symbol}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono text-text-secondary">{spotRate > 0 ? `1 ${TOKENS[tokenIn].symbol} = ${spotRate.toFixed(6)} ${TOKENS[tokenOut].symbol}` : '—'}</span>
              </div>
              {!use1inch && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={clsx(
                    ...
                    priceImpact < 0.5 ? 'text-emerald-400' :
                    priceImpact < 2.0 ? 'text-yellow-400' : 'text-red-400'
                  )}>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono text-text-secondary">{parseFloat(formatUnits(amountOutMin, TOKENS[tokenOut].decimals)).toFixed(6)} {TOKENS[tokenOut].symbol}</span>
              </div>
              {needsUnwrapStep && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-text-muted">Settlement</span>
                  <span className="font-mono text-2xs text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">1-Click Swap & Native ETH Unwrap</span>
                </div>
              )}
            </div>
          </div>
